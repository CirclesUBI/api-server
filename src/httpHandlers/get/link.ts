...
function onError(req: Request, res: Response, e: Error) {
  log(
    ...
    `The link preview couldn't be generated:`,
    e
  );
...
export const linkGetHandler = async (req: Request, res: Response) => {
  try {
    ...
    if (!id) return onError(req, res, new Error(`The 'id' query parameter is not set`));
    ...
    if (!linkObj) return onError(req, res, new Error(`Couldn't find a link with the specified id`));
      ...
      case "Business":
        ...
        if (businesses.length != 1)
          return onError(
            req,
            res,
            new Error(`Couldn't find a 'Business' with the specified 'circlesAddress' in the 'key' field`)
          );
...
<body>
        ...
        break;
      case "Person":
        const persons = await new ProfileLoader().profilesBySafeAddress(Environment.readonlyApiDb, [
          linkObj.linkTargetKey,
        ]);
