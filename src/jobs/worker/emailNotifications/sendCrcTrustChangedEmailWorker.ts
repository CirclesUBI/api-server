...
export class SendCrcTrustChangedEmailWorker extends JobWorker<SendCrcTrustChangedEmail> {
  ...
  async doWork(job: SendCrcTrustChangedEmail): Promise<void> {
    const profilesResult = await (new ProfileLoader()
      .profilesBySafeAddress(Environment.readonlyApiDb, [job.user]));

    const recipient = profilesResult[job.user]?.emailAddress;

    if (!recipient) {
      console.warn(`Couldn't send a notification email to profile ${profilesResult[job.user]?.id} because it has no email address.`);
      return;
    }

    await Mailer.send({
      bodyHtml: htmlTemplate,
      subject: "New incoming trust!"
    }, {
