...
export class SendCrcReceivedEmailWorker extends JobWorker<SendCrcReceivedEmail> {
  ...
  async doWork(job: SendCrcReceivedEmail): Promise<void> {
    const profilesResult = await (new ProfileLoader()
      .profilesBySafeAddress(Environment.readonlyApiDb, [job.to]));

    const recipient = profilesResult[job.to]?.emailAddress;

    if (!recipient) {
      console.warn(`Couldn't send a notification email to profile ${profilesResult[job.to]?.id} because it has no email address.`);
      return;
    }

    await Mailer.send({
      bodyHtml: htmlTemplate,
      subject: "You received circles!"
    }, {
