...
import {JobDescription, JobType} from "./descriptions/jobDescription";
                ...
                await new Promise(async (resolve, reject) => {
                        ...
                        try {
                            if (msg.payload && msg.payload?.trim() != "") {
                                ...
                                while (processedJobs > 0) {
                                    processedJobs = await JobQueue.consume(msg.channel.toLowerCase(), jobSink);
                                }
                            }
                        } catch (e) {
                            reject(e);
                    ...
                    if (isRunning) {
                        for (let topic of topics) {
                            ...
                            let processedJobs = 1;
                            while (processedJobs > 0) {
                                processedJobs = await JobQueue.consume(topic.toLowerCase(), jobSink);
                            }
                        }
