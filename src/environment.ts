...
export class Environment {
  static async validateAndSummarize() {
    ...
    if (!this.blockchainIndexerUrl) {
      throw new Error(`The BLOCKCHAIN_INDEX_WS_URL environment variable is not set.`);
    }
    ...
    if (!this.operatorOrganisationAddress) {
      throw new Error(`The OPERATOR_ORGANISATION_ADDRESS environment variable is not set.`);
    }

    console.log(`* Testing connection to the json rpc gateway (${this.rpcGatewayUrl}) ...`)
    const rpcGateway = await fetch(this.rpcGatewayUrl
      .replace("ws://", "http://")
      .replace("wss://", "https://"));
    if (rpcGateway.status.toString() == "200") {
      console.log("  Success. Body: " + (await rpcGateway.text()));
    } else {
      throw new Error(`The json rpc gateway responded with a non 200 code: ${rpcGateway.status}. Body: ${await rpcGateway.text()}`);
    }

    console.log("* Testing operatorOrganisationAddress ..")
    let nonce = await (new GnosisSafeProxy(RpcGateway.get(), RpcGateway.get().utils.toChecksumAddress(this.operatorOrganisationAddress))).getNonce();
    console.log(`  ${this.operatorOrganisationAddress} nonce is: ${nonce}`)

    if (!process.env.INVITATION_FUNDS_SAFE_ADDRESS) {
      throw new Error(`The INVITATION_FUNDS_SAFE_ADDRESS environment variable is not set.`);
    }
    ...
    console.log(`  ${this.verificationRewardFundsSafe.address} nonce is: ${nonce}`)

    if (!process.env.VERIFICATION_REWARD_FUNDS_KEY) {
      throw new Error(`The VERIFICATION_REWARD_FUNDS_KEY environment variable is not set.`);
    }

    if (!process.env.DIGITALOCEAN_SPACES_ENDPOINT ||
      !process.env.DIGITALOCEAN_SPACES_KEY ||
      !process.env.DIGITALOCEAN_SPACES_SECRET) {
      throw new Error(`The DIGITALOCEAN_SPACES_ENDPOINT, DIGITALOCEAN_SPACES_KEY or DIGITALOCEAN_SPACES_SECRET environment variable is not set.`);
    }
    ...
    if (indexerWsEndpoint.status.toString() == "200") {
      console.log("  Success. Body: " + (await indexerWsEndpoint.text()));
    } else {
      const body = await indexerWsEndpoint.text();
      throw new Error(`The indexer ws endpoint responded with a non 200 code: ${indexerWsEndpoint.status}. Body: ${body}`);
    }
  }
