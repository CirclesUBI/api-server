  ...
  return async (
    ...
    args.safeAddresses.forEach((o) => {
      ...
      if (cacheHit !== undefined) {
        if (cacheHit != null) results.push(cacheHit);
      } else {
    ...
    if (missingProfiles.length > 0) {
      ...
      if (missingProfiles.length > 0) {
        try {
          const circlesGardenProfiles = await findCirclesGardenProfiles(
            missingProfiles
          );
          circlesGardenProfiles
            .filter((o) => !!o)
            .forEach((o) => {
              results.push(o);
              if (o.circlesAddress) {
    ...
    if (loadContacts) {
      ...
      if (ownProfile.circlesAddress) {
        const _contacts = await contactsResolver(
          null,
          { safeAddress: ownProfile.circlesAddress },
          context
        );
    ...
    return results.map((o) => {
      return {
        ...
        displayTimeCircles:
          ownProfileId == o.id ? o.displayTimeCircles : undefined,
        trustsYou: o.circlesAddress
          ? contactsByAddress[o.circlesAddress]?.trustsYou
          : undefined,
        youTrust: o.circlesAddress
          ? contactsByAddress[o.circlesAddress]?.youTrust
          : undefined,
        lastEvent: o.circlesAddress
          ? contactsByAddress[o.circlesAddress]?.lastEvent
          : undefined,
      };
    });
  };
...
const requestsBySafeAddress: {
  [safeAddress: string]: WaitingCirclesGardenProfileRequest;
} = {};

async function flush() {
  Object.entries(requestsBySafeAddress)
    .filter((o) => o[1].finishedAt)
    .forEach((o) => {
      ...
      if (request.timeoutAt.getTime() < now.getTime()) {
        request.waitingPromises.forEach((waitingPromise) => {
          const timeoutError = new Error(
            `The request timed out at ${request.timeoutAt}`
          );
  ...
  fetch(requestUrl)
    ...
    .then((profiles: Profile[]) => {
      ...
      profiles
        ...
        .forEach((o) => (profilesBySafeAddress[<string>o.circlesAddress] = o));
      ...
      batch.forEach((o) => {
        ...
        if (!result) {
          console.log(
            `Couldn't find a circles.garden profile for safe ${o.safeAddress}`
          );
...
async function findCirclesGardenProfiles(
  ...
  const results = Promise.all(
    safeAddresses.map((safeAddress) => {
      let pendingRequest: WaitingCirclesGardenProfileRequest | null =
        requestsBySafeAddress[safeAddress];
