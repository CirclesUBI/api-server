...
export const mutationResolvers: MutationResolvers = {
  ...
  setIsFavorite: async (parent, args, context: Context) => {
    ...
    const existingFavorite = await Environment.readWriteApiDb.favorites.findFirst({
      where: {
        favoriteCirclesAddress: circlesAddress,
        createdByCirclesAddress: caller.profile.circlesAddress,
      },
    });
    const favoriteProfile = await new ProfileLoader().profilesBySafeAddress(Environment.readWriteApiDb, [
      circlesAddress,
    ]);
  ...
  shareLink: async (parent, args, context) => {
    const caller = await context.callerInfo;
    if (!caller?.profile?.circlesAddress) throw new Error(`Only profiles with a circlesAddress can share links.`);
    ...
    if (!RpcGateway.get().utils.isAddress(args.targetKey)) throw new Error(`'targetKey' must be an ethereum address`);
